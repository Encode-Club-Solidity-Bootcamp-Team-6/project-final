...
async function burnTokens(index: string, amount: string) {
  ...
  const token = await viem.getContractAt("LotteryToken", tokenAddress);
  const contract = await viem.getContractAt("Lottery", contractAddress);
  const allowTx = await token.write.approve([contractAddress, MAXUINT256], {account: accounts[Number(index)].account,});
  const receiptAllow = await publicClient.getTransactionReceipt({hash: allowTx,});
  console.log(`Allowance confirmed (${receiptAllow?.transactionHash})\n`);
  const tx = await contract.write.returnTokens([parseEther(amount)], {account: accounts[Number(index)].account,});
  const receipt = await publicClient.getTransactionReceipt({ hash: tx });
